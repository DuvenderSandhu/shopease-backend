...
export default function SignUp() {
  ...
  function ValidateBtn(){
    try{
      let result=SignupSchema.parse({email:email,password:password,fullName:fullName})
      if(result){
  ...
  .then(data => {
    data.type==='success'?ifSuccess(data):dispatch(actionCreators.showNotification({type:'alert',msg:data.msg}))

  })
  .catch(error => {
    dispatch(actionCreators.showNotification('alert',error.msg))
    console.error('Error:', error);
  });
      }
      
    }catch(e){
      console.log(Array.from(e.issues))
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <div className="mt-2">
                    ...
                    ></input>
                    <span className="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">{error.map(e=>e.path[0]=='fullName'?e.message:"")}</span>
                  </div>
                </div>
                <div>
